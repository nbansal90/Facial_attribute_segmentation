...
l = open("/home/bansa01/taleb/image_output.txt","wb")
...
def main():
	...
	dataset_train = HelenData('/home/bansa01/taleb/SmithCVPR2013_dataset_resized/','names.txt',transform=transform)
	...
	val_loader = torch.utils.data.DataLoader(dataset_train, batch_size=1,
		shuffle=True, **kwargs)
...
def validate(val_loader, model, criterion, epoch):
	...
	for i, (input, target, img) in enumerate(val_loader):
		target = target.cuda(async=True)
		input = input.float()
		input = input.cuda()
		with torch.no_grad():
